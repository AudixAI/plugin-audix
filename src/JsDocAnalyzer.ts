...
export class JsDocAnalyzer {
  ...
  */
    ...
    */
    ...
    private findNearestNode(ast: TSESTree.Program, lineNumber: number): TSESTree.Node | undefined {
        ...
        const traverse = (node: TSESTree.Node | null) => {
            ...
            if ('declarations' in node && Array.isArray(node.declarations)) {
                node.declarations.forEach(decl => traverse(decl));
            }
    ...
    /**
     * Gets the source code text for a node
     */
    private getNodeText(node: TSESTree.Node): string {
        // This is a simplified version - you'll need to implement proper node-to-text conversion
        // or use the original source code and node locations to extract the text
        return `${node.type} at line ${node.loc.start.line}`;
    }
}